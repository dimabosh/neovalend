...
async function deployConfiguratorAndProxy() {
    ...
    try {
        ...
        if (isNeoX) {
            ...
            deployCommand = `forge create "${contractPath}" --libraries ${libraryPath}:ConfiguratorLogic:${configuratorLogicAddress} --private-key ${process.env.DEPLOYER_PRIVATE_KEY} --rpc-url ${process.env.RPC_URL_SEPOLIA} --verify --verifier blockscout --verifier-url ${verifierUrl} --broadcast --json --use 0.8.27`;
